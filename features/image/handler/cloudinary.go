...
func UploadImage(c echo.Context) (string,error) {
	...
	if errParam != nil {
		return "", helper.FailedRequest(c, "failed config "+errParam.Error(), nil)
	}
	// Mengambil file dari permintaan
	file, err := c.FormFile("image")
	if err != nil {
		// Menangani error ketika gagal mendapatkan file
		return "", helper.FailedNotFound(c, "Failed to get image file from request", nil)
	}
	...
	if err != nil {
		...
		return "", helper.FailedRequest(c, "Failed to open file", nil)
	}
	defer fileSrc.Close()
	...
	if err != nil {
		...
		return "", helper.FailedRequest(c, "Failed to read file", nil)
	}
	...
	if err != nil {
		...
		return "", helper.FailedRequest(c, "Failed to upload file", nil)
	}
	log.Println(uploadResult.SecureURL)

	asset, errAsset := cld.Admin.Asset(ctx, admin.AssetParams{PublicID: file.Filename})
	if errAsset != nil {
		return "", helper.FailedNotFound(c, "failed asset "+errAsset.Error(), nil)
	}
	...
	if errSearch != nil {
		return "", helper.InternalError(c, "failed data result "+errSearch.Error(), nil)
	}
